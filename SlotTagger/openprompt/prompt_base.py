...
class Template(nn.Module):
    ...
    Most of the methods are abstract, with some exceptions to hold the common methods for all template, such as ``loss_ids``, ``save``, ``load``.

    Args:
        tokenizer (:obj:`PreTrainedTokenizer`): A tokenizer to appoint the vocabulary and the tokenization strategy.
        placeholder_mapping (:obj:`dict`): A placeholder to represent the original input text.
    '''
    ...
    def __init__(self,
        ...
        # self._in_on_text_set这个参数是为了避免同时处理，默认值是False代表没有在处理
    ...
    def get_default_shortenable_ids(self) -> List[int]:
        ...
        the language model's ``max_seq_length``. Default: the input text is shortenable, while the template text and others
    ...
    # 使用装饰器@property与setter，将该方法同时定义成getter与setter，这样既可以在读写属性时添加别的东西，又可以用类似属性的方式来读写类的变量。
    # 如果只设置@property，而不设置@setter，则等价于将该属性设置成只读。
    # 这两个方法，写在父类里面，但只在子类里面调用了。
    @property
    def text(self):
        return self._text

    # 在父类与子类中，只要给self.text赋值，则会触发这个方法。setter相比getter，会出一个参数，也就是等号右边的内容
    # 例如self.text = text,就会等价于self.set_text(text)
    @text.setter
    def text(self, text):
        self._text = text
        if text is None:
...
class Verbalizer(nn.Module):
    ...
    def label_words(self,):
        ...
        在这里，label是positive，label_word是wonderful、good等
        '''
    ...
    def generate_parameters(self, **kwargs) -> List:
        ...
        In others situation, the parameters may be a continuous vector over the
    ...
    def from_file(self,
        ...
        Args:
            ...
            verbalizer的choice指的是，如果提供多个verbalizer，选择哪一个，当然同时之只能使用1个
        Returns:
