...
class SoftVerbalizer(Verbalizer):
    ...
    def __init__(self,
        ...
        # mutil token handler ，好像在softverbalizer里面没有使用。

        head_name = [n for n,c in model.named_children()][-1]
        # 返回的子模块的迭代器的最后一个，该模块的名称为cls，指的应该是最后一层的线性层。
        ...
        if not isinstance(self.head, torch.nn.Linear):
            ...
            # 备份原始的head(线性层)，并且创建新的空的head(线性层)，(768,39)，也就是说特定于下层任务的

        if label_words is not None: # use label words as an initialization
            self.label_words = label_words


    # bert这类任务头只有一层线性层的，只有group2没有group1
    @property
    def group_parameters_1(self,):
        ...
        others than the final projection layer. The parameters of these part should be
    ...
    def generate_parameters(self) -> List:
        ...
        for word in self.label_words:
            ...
            # exit()
            # if len(word_ids) > 1:
                # multi token的情况下，使用multi_token_handler处理。
                # 也就是如果某个label word 本身就是由多个token组成，或者由一个复杂点的word组成被tokenizer给截断成几个token了，
        ...
        # 求单个词组的最大单词长度，计算padding所需要的mask矩阵，然后padding
        ...
        # label_words_ids的shape (39,9),(num_labels, max_len)

        # todo 搞明白这行代码，跟manual verb对比，这个像是在计算或者合并logits
        init_data = self.original_head_last_layer[self.label_words_ids,:] * self.label_words_mask.to(self.original_head_last_layer.dtype).unsqueeze(-1) # unsqueeze 拓展维度
        # self.original_head_last_layer = getattr(model, head_name).weight.data
        # original_head_last_layer是Bert自带的线性层的预训练好的权重矩阵，Bert的线性层维度是(768, 29886) (hidden_dims, vocab_size),权重矩阵的维度是其转置。
        # self.original_head_last_layer[self.label_words_ids,:]的维度是(39,9,768),self.label_words_mask.unsqueeze(-1)的维度是(39,9,1)，两者相乘的维度是(39,9,768),也就是上面的init_data
        init_data = init_data.sum(dim=1)/self.label_words_mask.sum(dim=-1, keepdim=True)
        # 下面的init_data的维度是(39, 768),主要作用是聚合单个词组中所有单词的logits。
        # (39，768) = (39,768)/(39,1)
        # 由此，得到新的线性层的权重矩阵，赋给空的线性层，参与训练与更新。
