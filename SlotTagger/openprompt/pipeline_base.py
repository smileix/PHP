...
class PromptModel(nn.Module):
    ...
    Args:
        ...
        plm_eval_mode (:obj:`bool`): this is a stronger freezing mode than freeze_plm, i.e. the dropout of the model is turned off. No matter whether the others part is set to train.
    '''
    def __init__(self,
                 plm: PreTrainedModel,
                 template: Template,
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        # 这里plm是既有encoder又有任务头的，输入进入直接获得PLM任务头的输出。

        # print(outputs.logits.shape)
        # logits的 shape是(bsz, max_seq_len, vocab_size),最后一维为什么是这个数字。bert的vocab_size是28996
        # print(outputs.hidden_states[-1].shape)
        # (bsz, max_seq_len, 768)
        # 此时outputs.loss还是None，可能是因为还没到计算loss的阶段。
        # exit()
        # outputs类型为transformers.modeling_outputs.MaskedLMOutput or tuple(torch.FloatTensor)
        # 取决于return_dict或者config.return_dict的设置
        # if return_dict=False is passed or when config.return_dict=False
        # MaskedLMOutput有4个属性，分别是loss，logits，hidden_states与attentions
        # tuple的四个元素分别是last_hidden_state, pooler_output, hidden_states与attentions
...
class PromptForClassification(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        ...
        # print(type(outputs))
        # 此处的output还是MAskedLMOutput类
        outputs = self.verbalizer.gather_outputs(outputs)
        # print(outputs.shape)
        # 此处的outputs已经是logits了，shape是(bsz, max_seq_len, 28996),具体来说就是(8,256,28996)
        # 28996是bert的词库大小，对所有28996个单词向量执行点积，以找出句子中哪个位置的向量输出与28996个单词向量的相似度最高
        # 简单起见，设bsz为1，m1 为(max_seq_len,hidden_states_dim), m2 为 (hidden_states_dim, vab_size)
        # m1 点乘 m2, 得到新矩阵(max_seq_len, vab_size)，也就是seq中每个单词的logits，
        # 训练时用该logits计算loss。推理时，对logits进行argmax，得出句子中每个位置最可能的单词
        # 这里的max_seq_len也可以优化成max_seq_len_based_batch，每个batch中的最大seq len，以减少显存占用。
        # 对于bert来说，vab_nums等于28996，也就是说，它的词典总共有28996个单词。
        if isinstance(outputs, tuple):
            outputs_at_mask = [self.extract_at_mask(output, batch) for output in outputs]
        else:
            outputs_at_mask = self.extract_at_mask(outputs, batch)
        # output的shape(``batch_size``, ``max_seq_length``, ``vocab_size``)
        # (if num_mask_token > 1)，则outputs at mask的shape为(``batch_size``, ``num_mask_token``, ``vocab_size``)
        # (if ``num_mask_token`` = 1)，则outputs at mask的shape为 (``batch_size``, ``vocab_size``).
        # print(outputs_at_mask.shape)
        # output_at_mask shape: (bsz, vocab_size),具体来说就是(8,28996)
        # exit()
        label_words_logits = self.verbalizer.process_outputs(outputs_at_mask, batch=batch)
        # 这里使用process_outputs方法，对logits进行处理，主要是将对于整个vocab_size的logits，压缩到classes上。
        # 也就是所，其输入是(bsz,vocab_size)(8,28996),处理成(bsz,class_num),其中每个class包括若干个label words。
