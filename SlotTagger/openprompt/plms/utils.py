...
class TokenizerWrapper:
    def __init__(self, max_seq_length: int, tokenizer: PreTrainedTokenizer, truncate_method: Optional[str] = 'tail',
                 create_token_type_ids: Optional[str] = False, **kwargs):
        ...
        if not hasattr(self, "_special_tokens_map"):
            ...
            for attrname in self.__dict__.keys():
                if attrname.endswith('_token_map'):
                    _special_tokens_map.update(
                        getattr(self, attrname))  # dict.update(dict2) 这里getattr(self, token_map)返回的是一个dict
        return _special_tokens_map

    def tokenize_with_mask(self, wrapped_example: List[Dict], ) -> InputFeatures:
        raise NotImplementedError

    def tokenize_without_mask(self, wrapped_example: List[Dict], ) -> InputFeatures:
        raise NotImplementedError
    ...
    def balanced_truncate(input_dict: Dict, num_tokens_to_truncate: int = 0) -> Dict:
        ...
        '''
        shortenable_lens = [len(parts) if parts[0] == 1 else 0 for parts in input_dict['shortenable_ids']]
        total_shortenable_len = sum(shortenable_lens)
        num_tokens_to_truncate_each_part = [part_len / total_shortenable_len * num_tokens_to_truncate for part_len in
                                            shortenable_lens]
    ...
    def concate_parts(input_dict: Dict) -> Dict:
        for key in input_dict:
            input_dict[key] = list(itertools.chain(*input_dict[
                key]))  # chain(*iterables) 可以把多个可迭代对象组合起来，形成一个更大的迭代器  # 对于单个key来说，其value可能是一个list，且list的每个元素是字符串，这样可以该list的所有元素组合起来，形成一个更大的迭代器
        return input_dict
        ...
        for key in encoder_inputs:
            if key == "input_ids":
                with warnings.catch_warnings():
                    ...
                    encoder_inputs[key] = self.tokenizer.build_inputs_with_special_tokens(encoder_inputs[key])
            else:
    ...
    def truncate(self, encoder_inputs):
        ...
        if num_tokens_to_truncate > 0:
            ...
            encoder_inputs = self.truncate_fct(input_dict=encoder_inputs, num_tokens_to_truncate=num_tokens_to_truncate)
        return encoder_inputs
