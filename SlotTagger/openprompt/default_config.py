...
def get_default_config():
    ...
    cfg.logging.unique_string_keys = ['dataset.domain', 'datetime'] # used to generate the unique string for saving
        #- dataset.name
    ...
    cfg.logging.datetime_format = "%H%M%S" # only useful when unique_string_keys includes `datetime`.
        #  make sure it's a valid format for datetime package.

    # 需要载入训练好的模型进行测试时，需要设置该path
    ...
    cfg.train.batch_size = 64 # the batch_size.
    ...
    # TODO 调参时可以尝试更改teacher_forcing
    ## teacher-forcing 在训练网络过程中，每次不使用上一个state的输出作为下一个state的输入，而是直接使用训练数据的标准答案(ground truth)的对应上一项作为下一个state的输入。
    ## teacher-forcing过于依赖ground truth数据，在训练过程中，模型会有较好的效果，但是在测试的时候因为不能得到ground truth的支持，
    ## 所以如果目前生成的序列在训练过程中有很大不同，模型就会变得脆弱。
    ...
    ## TODO 需要研究一下LMBFF的细节，看看是否能够借鉴使用
    cfg.classification.auto_t = False
    cfg.classification.auto_v = False
    # 应该分别表示auto template与auto verbalizer
    ...
    cfg.dataset.domain = None  # for snips datasets & cross domain setting
    cfg.dataset.use_true_bio_label = False


    ## DATALOADER ######################################################
    cfg.dataloader = CfgNode(new_allowed=True)
    cfg.dataloader.max_seq_length = 64 # max_seq_length
    cfg.dataloader.decoder_max_length = 128 # the decoder max length to truncate decoder input sequence
    ...
    cfg.manual_verbalizer.multi_token_handler = 'mean'
