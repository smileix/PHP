...
class InputExample(object):
    ...
    def __init__(self, guid=None, text_a="", text_b="", label=None, meta: Optional[Dict] = None,
                 tgt_text: Optional[Union[str, List[str]]] = None, ys=None, span=None, ):
        self.guid = guid
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
...
class InputFeatures(dict):
    ...
    """
    # 取消设置label为tensorable key，原先的label为list，为了便于传递其他所需标注，将其更改为dict，这样的话需要自己在后面计算loss时，取出原先的label并转为tensor
    tensorable_keys = ['input_ids', 'inputs_embeds', 'attention_mask', 'token_type_ids', 'decoder_input_ids',
                       'decoder_inputs_embeds', 'soft_token_ids', 'past_key_values', 'loss_ids']
    all_keys = ['input_ids', 'inputs_embeds', 'attention_mask', 'token_type_ids', 'label', 'decoder_input_ids',
                'decoder_inputs_embeds', 'soft_token_ids', 'past_key_values', 'loss_ids', 'guid', 'tgt_text',
                'encoded_tgt_text', 'input_ids_len']
    non_tensorable_keys = []

    def __init__(self, input_ids: Optional[Union[List, torch.Tensor]] = None,
                 ...
                 loss_ids: Optional[Union[List, torch.Tensor]] = None, guid: Optional[str] = None,
                 tgt_text: Optional[str] = None, use_cache: Optional[bool] = None,
                 encoded_tgt_text: Optional[str] = None, input_ids_len: Optional[int] = None, ys=None, span=None,
                 **kwargs):
        ...
        for k in kwargs.keys():
            logger.warning(
                "Your are passing an unexpected key words: {} to InputFeatures, might yield unexpected behaviours!".format(
                    k))
