...
def load_dataset(config: CfgNode, return_class=True, test=False):
    ...
    processor = PROCESSORS[dataset_config.name]()

    train_dataset = None
    valid_dataset = None
    if not test:
        try:
            train_dataset = processor.get_train_examples(dataset_config.path, domain=dataset_config.domain if dataset_config.domain else None,
                                                         use_true_bio_label=dataset_config.use_true_bio_label)
        except FileNotFoundError:
            logger.warning(f"Has no training dataset in {dataset_config.path}.")
        try:
            valid_dataset = processor.get_dev_examples(dataset_config.path, domain=dataset_config.domain if dataset_config.domain else None,
                                                       use_true_bio_label=dataset_config.use_true_bio_label)
        except FileNotFoundError:
    ...
    test_dataset = None
    try:
        test_dataset = processor.get_test_examples(dataset_config.path, domain=dataset_config.domain if dataset_config.domain else None,
                                                   use_true_bio_label=dataset_config.use_true_bio_label)
    except FileNotFoundError:
